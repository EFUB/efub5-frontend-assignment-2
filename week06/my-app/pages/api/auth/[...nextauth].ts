...
export const authOptions: AuthOptions={
    providers:[
        ...
        CredentialsProvider({ name: "credentials", credentials: {
            ...
            async authorize(credentials: any): Promise<any> { if (!credentials) {
            console.log("credentials가 제공되지 않았습니다.");
            return null; 
            }
            let db = (await connectDB).db("forum"); let user = await db
            .collection("user_cred")
            .findOne({ email: credentials.email });
            if (!user) {
            console.log("해당 이메일은 없음"); return null;
            }
            const pwcheck = await bcrypt.compare( credentials.password,  user.password
            );
            if (!pwcheck) { console.log("비번틀림"); return null;
            }
            return user;
            },
        }),
